...
namespace VkApiSDK.Polls
{
    ...
    public class PollMethdos : VkApiMethodGroup
    {
        ...
        /// <returns></returns>
        public async Task<VkResponse<int>> AddVote(Poll Poll, IEnumerable<VoteAnswer> Answers)
        {
        ...
        /// <returns></returns>
        public async Task<VkResponse<VotersData[]>> GetVotes(Poll Poll, IEnumerable<VoteAnswer> Answer, int Offset = 0, int Count = 40, bool FriendsOnly = false, bool IsBoard = false)
        {
            return await _vkRequest.Dispath<VkResponse<VotersData[]>>(
                new GetVoters(
                    AccessToken: AuthData.AccessToken,
                    OwnerID: Poll.OwnerID,
                    PollID: Poll.ID,
        ...
        public async Task<VkResponse<Poll>> CreateVote(string QuestionText, IEnumerable<string> Answers, bool IsAnanimus = false, bool IsMultipleVote = false, long EndDate = 0, int BackgroundID = 1)
        {
            return await _vkRequest.Dispath<VkResponse<Poll>>(
                new CreateVote(
                    AccessToken: AuthData.AccessToken,
